...
`constraints.optimization`은 기존 문서 안에서 허용되는 연출 범위를 기록한다. 기본값은 `wording: allowed`, `emphasis: allowed`, `reorder_within_section: allowed`, `resequence_repeated_units: allowed`다. `structural_redesign`과 `add_or_remove_content_blocks`는 사용자가 명시적으로 승인하기 전 `require-user-approval`이다.
...
4. **Gate 3 - Matching Strategy**: 역량 정리, 요구사항-근거 매트릭스, 회사 신호-지원자 근거 매트릭스, 매칭 전략이 서로 일치한다. 직무 요건·인재상·문화·업무 방식·제품 맥락의 사실과 해석이 구분되고, 최종 어필 방향이 검증된 지원자 근거에 연결됐다.
5. **Gate 4 - Application Drafts**: 문서별 분석·연출 계획, 최종 초안, 블록 단위 변경 이력과 형식 보존 비교가 완성됐다.
...
- `source-locked`는 원본의 형식 골격, 섹션 종류, 블록 템플릿, 표 구조와 자산 연결을 보존한다. 매칭 전략과 근거가 있으면 같은 섹션 안의 글머리표와 기존 경력·프로젝트 같은 반복 단위를 통째로 재배치하고, 표현·강조·분량을 조정할 수 있다. 모든 실질적 변경은 회사 신호 ID와 지원자 근거 ID를 `change-log.md`에 기록한다.
- 섹션·프로젝트·문단의 추가나 삭제, 블록 병합·분할, 최상위 섹션 재배치, 서로 다른 부모 간 블록 이동, 표·페이지 틀 변경은 구조 변경이다. 사용자 승인이나 명시적 제출 규격 없이는 최종본에 적용하지 않는다.
...
- JSON 입력은 키 이름, 중첩, 키 순서, 값 타입과 배열 항목 구조를 유지한다. 기존 경력·프로젝트 같은 표시 단위의 배열 순서는 전략적 재배치로 바꿀 수 있으나 항목을 추가·삭제하거나 내부 구조를 바꾸지 않는다.
- HTML 입력은 의미 구조, 클래스와 자산 연결을 유지하며 같은 부모 아래 반복 단위의 DOM 순서만 전략적으로 바꿀 수 있다. DOCX/PDF는 관찰 가능한 문단, 표, 이미지와 페이지 구조를 기준으로 삼는다.
...
- `application/company-signal-evidence-matrix.md`
...
- 검수는 프로필 버전·근거 ID, 사실성, 상충 정보, 회사 조사 목차, 회사 신호-지원자 근거 매핑, 면접 포맷, 원본 형식 대응, 전략적 재배치의 추적성, 회사명 사용, ATS 가독성, 날짜·연락처·링크, 표와 이미지 누락, 내부 표식 노출을 검사한다.
...
- 모든 `tailor` deliverable이 원본 `source_id`와 연결되고 형식 보존 비교를 통과했다. 전략적으로 바뀐 어휘·강조·순서는 회사 신호와 지원자 근거에 연결돼 있다.
