...
4. source와 최종 결과를 형식 골격, 섹션 수, 표·목록·이미지·캡션 관계 기준으로 비교하고 전략적 재배치와 구조 변경을 구분한다.
5. 회사 조사와 면접 자료를 각각의 report contract와 대조한다.
6. 회사 신호-지원자 근거 매트릭스와 matching strategy가 최종본보다 먼저 작성됐는지 확인한다.
7. 실질적인 어휘·강조·순서 변경을 company signal ID와 evidence ID에 역추적하고 최종 제출 문장의 사실과 수치를 evidence ledger에 대조한다.
8. findings를 심각도순으로 기록하고 verdict를 마지막에 결정한다.
...
- **Source fidelity**: 제목 위계, 섹션 종류, 블록 템플릿, 문단 경계, 목록·표 구조, 이미지·프로젝트 연결과 캡션이 preservation 규칙과 일치한다.
- **Strategy traceability**: 최종 어필 방향과 실질적인 어휘·강조·순서 변경이 source-backed company signal과 verified candidate evidence에 연결된다.
- **Strategic sequencing**: 같은 섹션의 글머리표와 반복 단위 재배치는 matching strategy와 change log에 근거가 있고 형식 골격이나 자산 연결을 바꾸지 않는다.
- **Unauthorized changes**: 삭제, 추가, 병합, 분할, 최상위 섹션 재배치와 부모 간 이동이 change log의 이유와 승인 상태 없이 적용되지 않았다.
...
- **Requirement coverage**: 모든 요청 산출물과 주요 직무 요구사항을 다뤘고, 확인 가능한 인재상·문화·업무 방식·제품 맥락을 지원자 강점에 매핑했다.
...
- **Assets**: 이미지 경로, 프로젝트 연결, 프로젝트 내부 순서, 캡션과 비율이 보존된다.
...
- `Major`: 원본 구조의 무단 변경, 회사 신호·지원자 근거 매핑 없는 임의의 어휘·강조·재배치, 중요 섹션·표·이미지 누락, 회사 조사 고정 섹션 누락, 면접 scope·질문 계약 미충족, unsupported claim, 중요 요구사항 공백, 내부 표식 노출.
...
`quality-review.md`는 `Findings -> Gate Evidence -> Contract Coverage -> Source Fidelity -> Strategy Traceability -> Evidence And Truth -> Submission Checks -> Verdict -> Fix List` 순서로 작성한다.
